...
BOOL CRenameErrorDlg::OnInitDialog()
{
	...
	{
		int nInsertedIndex = m_ctlReport.InsertItem(i, m_vErrors[i].fnBefore.GetFullPath());
		m_ctlReport.SetItem(nInsertedIndex, 1, LVIF_TEXT, m_vErrors[i].fnAfter.GetFullPath(), NULL, NULL, NULL, NULL);
